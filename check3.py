...
class EveTest(object):
    ...
    """
    def __init__(self, pk_maps={}, default_pk="_id"):
        ...
        resource_regex = re.compile("http://localhost/api/([a-zA-Z0-9-_]+)/")
        ...
        item_regex = re.compile("http://localhost/api/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)/")
...
"""
with EveTest(default_pk="key"):
    r = requests.post("http://localhost/api/omg/",
        {"item": json.dumps({"key": "ok", "test": "mdr"})})
    print "#########"
    d = r.json()
    print d
    etag = d["item"]["etag"] + "ok"

    r2 = requests.get("http://localhost/api/omg/")
    print r2.json()
"""